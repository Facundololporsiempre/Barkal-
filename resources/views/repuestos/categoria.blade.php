    ...
    <div class="bg-zinc-200 py-16">
        <div class="grid grid-cols-4">
            <div class="col-span-4 order-1 mb-6 md:col-span-1 md:order-2 mx-4 md:ml-12 lg:ml-20">
                <div class="mb-10">
                    <h1 class="text-gray-900 text-4xl leading-4 font-medium">{{$categoria->name}}</h1>
                    <p class="text-gray-400 text-sm leading-4 pt-4">{{$cantidad}} resultados</p>
                </div>
                <div class="mb-10">
                    <h2 class="text-gray-900 leading-4 font-medium">Categoría</h2>
                    <p class="text-gray-400 text-sm leading-4 pt-2">{{$categoria->name}}</p>
                </div>
                <div class="mb-10">
            ...
            {{-- !!! NO BORRAR ESTO !!! --}}
                {{-- <div>
                    <form action="{{route("repuestos.categoria", $categoria)}}" method="get">
                        @csrf
                        <div class="mb-1">
                            <label class="text-gray-900 leading-4 font-medium">Precio a buscar</label>
                        </div>
                        <div class="flex">
                            <input class="w-28 rounded-l-lg" type="text" name="price" value="{{$price}}" placeholder="Ingrese el precio a buscar" autocomplete="off">
...
@else
    <div class="bg-white grid grid-cols-2 p-6 mx-8 md:mx-32 lg:mx-60 my-16 rounded-lg">
        ...
        <div class="col-span-2 md:col-span-1 flex md:items-center">
            <strong><p class="text-center md:text-left text-2xl">No hay actualmente repuestos en la categoria "{{$categoria->name}}"</p></strong>
        </div>
        <div class="col-span-2 md:col-span-1">
        </div>
        <div class="col-span-2 md:col-span-1">
            <ul>
                <li class="text-gray-900 text-sm my-2">-Pronto agregaremos repuestos en esta categoria.</li>
            </ul>
        </div>
    </div>
@endif
